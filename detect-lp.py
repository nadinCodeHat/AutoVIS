...
if lp_detected == True:
    ...
    cv2.imwrite('./croppedimages/7.png', plate_image)
    # Store new image
    # cv2.imwrite('C:\\Users\\LENOVO\\Documents\\Projects\\yolov5\\croppedimages\\' +
...
corrected_img = cv2.resize(corrected_img, None, fx=3,  # type: ignore
...
if -8 <= angle <= -4:
    ...
    # cv2.waitKey(0)
# else:
    #cv2.imshow("3X Image", corrected_img)
    # cv2.waitKey(0)
cv2.imwrite('./croppedimages/8.png', corrected_img)


cv2.destroyAllWindows()


call(["python", "recognize.py"])
